...
public class StringUtil {
	...
	/**
	 *
	 * @Description 如果obj是null，则返回空字符串""，否则返回obj.toString()。
	 * @param obj
	 * @return
	 *
	 */
	 ...
	 *            要除去的字符，如果为<code>null</code>表示除去空白字符
	 * @param mode
	 *            <code>-1</code>表示trimStart，<code>0</code>表示trim全部，<code>1</code>表示trimEnd
	 *
	 * @return 除去指定字符后的的字符串，如果原字串为<code>null</code>，则返回<code>null</code>
	 */
...
//	}
	 ...
	 * 如果字符串集合为<code>null</code>或空，也返回<code>-1</code>。 如果字符串集合包括<code>""</code>，并且字符串不为<code>null</code>，则返回<code>str.length()</code>
	 * <pre>
	 * StringUtil.indexOfAny(null, *)                     = -1
	 * StringUtil.indexOfAny(*, null)                     = -1
	 * StringUtil.indexOfAny(*, [])                       = -1
	 ...
	 *
	 * <p>
	 * 如果字符串为<code>null</code>，则返回<code>false</code>。 如果字符集合为<code>null</code>则返回<code>false</code>。
	 ...
	 * <li> 如果<code>maxWidth</code>小于<code>4</code>抛出<code>IllegalArgumentException</code>；
	 * </li>
	...
	/**
	 * 去掉头尾，比如：[hello] 处理后为：hello
	 */
	public static final String subStringHeadTail(String s){
		if (s == null) {
			return null;
		}
	...
	/**
	 * 从某个位置开始替换成某个字符串，如： samsam22 替换成 sa*****
	 */
	public static final String replaceStart(String s,String replaceChar, int start){
		if (s == null) {
			return null;
		}
		...
		tmpStr.append(s.substring(0, start));

		for (int i = start; i < s.length(); i++) {
			tmpStr.append(replaceChar);
		}
	 ...
	 * @return
	 */
	protected String ids2SQLInString(Collection<Object> ids) {
	 ...
	 *
	 */
	public static String doLogStr(String updateBefore,String updateAfter){
		//日志详情
		StringBuilder logDetail= new StringBuilder();
		//组装日志
		logDetail.append("修改前[");
		logDetail.append(updateBefore);
		logDetail.append("],");

		logDetail.append("修改后[");
		logDetail.append(updateAfter);
		logDetail.append("]");
	...
	/**
	 * 裁剪指定字符串到指定长度，字符串右端用"..."填充：
	 * 1、如果要操作的字符串的长度小于截取的长度，不做操作，返回原字符串
	 * 2、如果截取长度len <= 3:只截取，不再用"..."填充
	 * 例1："abcdefghijklmnop" -->   设:len = 10
	 * 结果：                                             -->   "abcdefj..."（length=10）
	 *
	 * 例2："abcdefg" --> 设:len=10
	 * 结果                             --> "abcdefg"
	 *
	...
	/**
	 * 裁剪指定字符串到指定长度，字符串中间用"..."填充：
	 * 1、如果要操作的字符串的长度小于截取的长度，不做操作，返回原字符串
	 * 2、如果截取长度len <= 3:只截取，不再用"..."填充
	 * 例1："abcdefghijklmnop" -->   设:len = 10
	 * 结果：                                                -->   "abcd...efj"（length=10）
	 *
	 * 例2："abcdefg" --> 设:len=10
	 * 结果                             --> "abcdefg"
	 *
	 ...
	 * @return
	 */
	public static String buZero(long lo,int len){
	...
	public static String formatLongNumber(Long number, Integer length) {
		return String.format("%0"+length+"d", number);
	}

	public static String collectionToString(Collection<String> collection){
		if(collection==null||collection.size()==0) return EMPTY_STRING;
